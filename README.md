...
Автор: Фролов Иван Денисович. Telegram: [@lxskx](https://t.me/lxskx), почта: [frolivand@icloud.com](mailto:frolivand@icloud.com), GitHub: [lxskx](https://github.com/lxskx).
...
1. Клонируйте репозиторий:
   ...
   ```bash
   git clone https://github.com/kkchillbro/teamfinder-university-demo.git
   ```

2. Перейдите в папку проекта:

   ```bash
   cd teamfinder-university-demo
   ```
...
```bash
DJANGO_SECRET_KEY=dev-secret DJANGO_DEBUG=True USE_SQLITE=True python manage.py migrate
DJANGO_SECRET_KEY=dev-secret DJANGO_DEBUG=True USE_SQLITE=True python manage.py seed_data
DJANGO_SECRET_KEY=dev-secret DJANGO_DEBUG=True USE_SQLITE=True python manage.py runserver
```
...
```bash
DJANGO_SECRET_KEY=dev-secret DJANGO_DEBUG=True USE_SQLITE=True python manage.py test
```
...
- Папки `templates_var1` и `templates_var2` оставлены как материалы стартового набора; приложение использует актуальный набор `templates_var3`.
